...
class _AiSupportScreenState extends State<AiSupportScreen> {
  ...
  Future<void> _send() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() {
        _messages.add(_AiMessage(role: _AiRole.assistant, text: reply.isEmpty ? '...' : reply));
      });
    } catch (e) {
      if (!mounted) return;
      setState(() {
        _messages.add(_AiMessage(role: _AiRole.assistant, text: 'Đã có lỗi xảy ra: ${e.toString()}'));
      });
    } finally {
      if (!mounted) return;
      setState(() {
                ...
                return Align(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(color: isUser ? const Color(0xFF17a64b) : const Color(0xFFE2E8F0)),
                    ),
